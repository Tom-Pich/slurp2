...
class ObjectController
{
	...
	public const object_types = [
		"générique" => [
			"localisations" => [
				"générale" => [1 => "Localisation 1", 2 => "Localisation 2", 3 => "Localisation 3", 4 => "Localisation 4", 5 => "Localisation 5", 6 => "Localisation 6"],
			]
		],
		"voiture" => [
			"localisations" => [
	...
	static public function getObjectDamageEffects(int $pdsm, int $pds, int $integrite, int $rd, int $rawDamages, string $dmgType, string $objectType, string $localisation, array $rolls)
	{
		...
		elseif ($netDamages <= .5 * $pdsm) $damageIndex = 3;
		elseif ($netDamages <= $pdsm) $damageIndex = 4;
		elseif ($netDamages <= 2 * $pdsm) $damageIndex = 5;
		else $damageIndex = 6;

		// located damages don’t affect the object $pds
		...
		elseif ($pds <= 0.75 * $pdsm) $stateLevelIndex = 2;
		elseif ($pds <= 0.9 * $pdsm) $stateLevelIndex = 1;
		else $stateLevelIndex = 0;

		// assessing side effects number
		$sideEffectNumber = 0;
		if ($dmgType === "localises") $sideEffectNumber = random_int(0, 1); // 0 or 1 if "très localisé"
		elseif ($stateLevelIndex <= 3) $sideEffectNumber = random_int(0, $damageIndex); // 0 to $damageIndex (0-4) if state is not "hors-service"
