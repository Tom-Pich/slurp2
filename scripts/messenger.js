	...
	constructor(id, login, key, serverURL, dialogWrapper, usersWrapper) {
		...
		this.wsClient.onopen = function () {
			...
			if (messenger.id) {
				...
				document.querySelector("#connected-users").innerHTML = "<div class=\"ta-center italic\">Vous n’êtes pas connecté.</div>";
				document.querySelector("#chat-dialog-wrapper").innerHTML = "";
				document.querySelector("[data-role=emojis-wrapper]").style.display = "none";
		...
		this.send = function (type, content) {
			...
			if (this.readyState === 1) {
				...
				if (!this.id) {
					const inputEntry = document.querySelector("#msg-input");
					const chatBox = document.querySelector("#chat-dialog-wrapper");
