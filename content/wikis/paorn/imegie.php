<h1>Imégie, capitale de l’Artaille</h1>

<div class="wiki-right-element">
	<figure>
		<a href="/assets/img_paorn/imegie-01.webp?v=2.2" target="_blank">
			<img src="/assets/img_paorn/imegie-01.webp?v=2.2">
		</a>
		<figcaption>Plan de la ville</figcaption>
	</figure>
	<figure>
		<img src="/assets/img_paorn/imegie-02.jpg" />
		<figcaption>Imégie – vieille ville</figcaption>
	</figure>
</div>

<img src="/assets/img_paorn/blason-imegie.png" width="70px" class="float-left">
<p>Cité portuaire aux maisons blanchies à la chaux, Imégie (également nommée la <i>Ville blanche</i>) est la capitale de l’Artaille.</p>

<!-- Géographie -->
<section>

	<h2>Géographie</h2>

	<p>Imégie est la ville la plus peuplée de l’Artaille, avec 25.000 habitants.</p>

	<p>Elle s’étend sur une superficie de 175 ha (2,6 km dans sa plus grande longueur, de la porte de Mineurs au nord-est jusqu'aux murailles de St-Almer au sud-ouest). La citée est divisée en deux partie : la vieille ville, ou ville haute, au sud-ouest, et la ville basse. Ces deux parties sont séparées par une fortification intérieure et communiquent par la <i>Vieille Porte</i>.</p>

	<p>La <i>Campua</i>, rivière prenant sa source dans les monts Hizrak, au nord, traverser la ville dans une gorge étroite.</p>

	<!-- Ville haute et ville basse -->
	<details>
		<summary>
			<h3>Ville haute et ville basse</h3>
		</summary>
		<p>Imégie est divisée en deux parties : la vieille ville et la ville basse.</p>
		<p>La vieille ville (ou ville haute) est ceinte par une muraille intérieure. Elle se décompose en quatre quartiers : la <i>Citadelle</i>, le <i>quartier des Consuls</i> juste après (en rouge), <i>Bellevue</i> (en vert), et enfin <i>Saint-Almer</i> à l'extrême sud-ouest (en jaune). La vieille ville a une superficie de 60 ha environ, et compte 8000 habitants. C’est la partie la plus riche de la ville et la sécurité y est particulièrement renforcée.</p>
		<p>La ville basse, correspondant à peu près à la moitié nord-est de la ville, comprend le port et la place du marché, où se trouve la cathédrale</p>
	</details>

	<!-- Quartiers -->
	<details>
		<summary>
			<h3>Quartiers</h3>
		</summary>
		<div class="wiki-right-element flow">
			<figure>
				<img src="/assets/img_paorn/imegie-rue-ville-haute.jpg" />
				<figcaption>Une rue de la vieille ville</figcaption>
			</figure>
		</div>
		<p>La ville compte 17 quartiers différents, plus la zone de l'université qui jouit d’un statut spécial.</p>
		<ul class="flow">
			<li><b>Quartiers des Consuls :</b> il abrite des bâtiments officiels (tribunal, les douanes, palais du peuple &ndash; pour les services administratifs de la ville : impôts, voirie, offices divers)</li>
			<li><b>Quartier de la Citadelle :</b> il s'agit essentiellement d'un quartier militaire (avec la caserne principale de la ville). On y trouve également les Écuries du Prince.</li>
			<li><b>Les trois buttes :</b> premier quartier par lequel on rentre dans la ville en arrivant par la route principale. Beaucoup d'auberges, des entrepôts, un poste de garde important, des écuries, la Grande Halle (voir plus loin). 9 ha</li>
			<li>
				<b>Le quartier du marché :</b> 11 ha. On y trouve le grand marché, mais aussi la cathédrale et un ensemble de bâtiments appartenant au clergé (palais episcopal, abritant les appartements du Grand Prêtre, l’administation cléricale de l’Artaille ainsi qu’une garnison de paladins – superficie 2 ha), ce qui en fait le centre névralgique de l’organisation du clergé atrimiste.<br>
				Distances entre la place du marché et : la Grande Porte (400 m), la Vieille porte (720 m), les quais du port (220 m), la porte des Mineurs (1000 m)
			</li>
			<li>
				<b>Le port :</b> environ 9 ha et 1330 habitants. Les quais se trouvent à 650 m de la Grande Porte.<br>
				Dans les alentours immédiats des quais, on trouve la Capitainerie (où sont gérés tous les aspects liés à la circulation et à l’amarrage des bateaux), les Douanes, l’Amirauté (qui gère les trois vaisseaux de guerre du Prince), ainsi que des entrepôts (notamment ceux des famille <i>Pelren</i> et <i>Brax</i>).
			</li>
			<li>
				<b>Les Andars :</b> quartier pauvre. On y trouve le port des pêcheurs, qui n'est en fait qu'une plage de galets. 12 ha.<br>
				La plupart des maisons ne comptent qu’un niveau. Elles sont défraîchies. La plupart des rues ne sont pas pavées, et celles qui le sont ne sont pas en bon état. Le quartier n’est pas sûr la nuit.
			</li>
			<li><b>Martilès :</b> quartier où se trouve la <i>Porte des Mineurs</i>. C’est un quartier pauvre où l’on trouve beaucoup d’étrangers venus travailler dans les mines. 13 ha.</li>
			<li><b>Hobbitbourg :</b> habité exclusivement par des hobbits, il se trouve sur les flancs de la montagne au Nord de la ville. C'est un quartier pauvre, et la majeure partie des habitations sont à moitié creusées dans la montagne. 13 ha pour 1800 habitants.</li>
		</ul>
	</details>

	<!-- Portes de la ville -->
	<details>
		<summary>
			<h3>Portes de la ville</h3>
		</summary>
		<p><b>La Grande Porte :</b> porte principale, située au nord-ouest de la ville, que l’on trouve en venant du reste du duché.</p>
		<p><b>La Porte des Mineurs :</b> au nord-est de la ville. Principalement emprûntée par les mineurs et les pêcheurs habitants dans les hameaux au nord de la ville.</p>
		<p><b>La Vieille Porte :</b> aussi appelée la <i>Porte de la vieille ville</i> sépare la vieille ville de la ville basse. Elle reste en principe ouverte de jour comme de nuit et est constamment surveillée par deux gardes.</p>
	</details>

	<!-- Place du marché -->
	<details>
		<summary>
			<h3>Place du marché</h3>
		</summary>
		<div class="wiki-right-element flow">
			<figure>
				<img src="/assets/img_paorn/imegie-marche-01.jpg" />
				<figcaption>Place du marché d’Imégie</figcaption>
			</figure>
			<figure>
				<img src="/assets/img_paorn/imegie-marche-02.jpg" />
				<figcaption>Cathédrale, place du marché</figcaption>
			</figure>
		</div>
		<p>Au cœur de la ville basse, la place du marché s'étend sous le regard imposant de la cathédrale. Chaque matin, ce lieu devient le centre névralgique de la cité, où l'activité commence dès les premières lueurs de l'aube.</p>
		<p>Entourés de façade à colombage, de clochers et de hauts bâtiments, de majestueux platanes centenaires offrent une ombre bienfaisante, sous laquelle des bancs en pierre accueillent les anciens. Au centre, les eaux cristallines d’une grande fontaine sculptée apportent une fraîcheur bienvenue. Non loin de là, un lavoir résonne des paroles et des chants des lavandières.</p>
		<p>Autour de la place, des tavernes, pâtisseries et auberges bordent les rues, leurs enseignes colorées invitant les passants à s'arrêter. Les étals des marchands débordent de marchandises variées : fruits et légumes, étoffes chatoyantes, épices exotiques. Les badauds se pressent, les clients marchandent, et les mendiants tendent la main, espérant une pièce ou un morceau de pain et de nombreux stands proposent toute sorte de plats préparés sur place.</p>
		<p>Des artistes de rue – jongleurs, musiciens, crieurs publics – captivent les foules par leurs talents et leurs histoires. Les enfants jouent à cache-cache entre les étals, leurs rires se mêlant aux cris des vendeurs. Quelques gardes, en faction, veillent discrètement à la sécurité, leurs regards scrutateurs balayant la foule.</p>
		<p>La place est également le royaume des animaux. Des chats et des chiens errent, profitant des restes de nourriture. Des ânes et des mulets, chargés de marchandises, avancent lentement, tandis que poules, moutons et cochons attendent d'être vendus.</p>
		<p>Les odeurs se mêlent et se superposent : les effluves appétissants des plats préparés sur place, les senteurs des légumes et des fruits frais, l’odeur plus âcres des animaux et du sang, et la fumée des feux de bois qui crépite dans l'air.</p>
		<p>Une partie de la place est occupée par des halles couvertes, offrant un abri contre le soleil brûlant et les intempéries. Sous ces toits protecteurs, la foule se presse encore plus dense, les voix résonnent, amplifiées par l'acoustique des lieux.</p>
	</details>

	<!-- La Grande Halle -->
	<details>
		<summary>
			<h3>La Grande Halle</h3>
		</summary>
		<div class="wiki-right-element">
			<figure>
				<img src="/assets/img_paorn/grande-halle-01.jpg" />
				<figcaption>La grande halle</figcaption>
			</figure>
		</div>
		<p>Située dans le quartier des trois collines, c’est l’endroit où transitent toutes les marchandises entrant ou sortant d’Imégie par voie terrestre.</p>
		<p>C’est un grand bâtiment avec une large cour intérieure où les chariots sont déchargés de leur marchandise puis rechargés, où les marchandises sont contrôlées par les autorités, les bêtes nourries et soignées&hellip;</p>
		<p>L’endroit est bien entendu solidement gardé, de jour comme de nuit</p>
	</details>

	<!-- Autres bâtiments notables -->
	<details>
		<summary>
			<h3>Autres bâtiments notables</h3>
		</summary>
		<ul>
			<li><b>Université</b> d’Opalia, réputée dans toute la Burgonnie.</li>
			<li><b>Château du prince</b>, grosse bâtisse aux tours rondes et aux remparts bas, adossée à des contreforts rocheux dans la vieille ville. À l'intérieur des deux premiers murs d'enceintes logent les nobles aux services du Prince, ainsi que leurs conseillers, plus des nobles n'exerçant aucune charge particulière.</li>
			<li><b>Domus Veritas :</b> QG de l’Arcania, petite fortesse monobloc au pied du château du Prince.</li>
			<li><b>Cathédrale de la Sainte-Lumière :</b> jouxtant la place du marché au centre de la ville.</li>
			<li><b>Capitainerie :</b> ce bâtiment héberge la capitainerie du port marchand, mais aussi les douanes. C’est un bâtiment à un seul étage, flanqué d’une tour. L’accès se fait obligatoirement par une lourde double porte de bois donnant dans une large cour intérieure.</li>
		</ul>
	</details>
</section>

<!-- Habitants -->
<section>
	<h2>Habitants</h2>
	<p>Comme dans tous l’Artaille, Imégie est peuplée à une très grande majorité d’humains. On y trouve également des hobbits (7 %). Quelques nains, la plupart ingénieurs dans les mines, fréquentent également la ville.</p>

	<p>Les Imégiens sont plutôt sympathiques, même s’ils ont une assez haute opinion d’eux-mêmes. Ils aiment faire la fête.</p>

	<?php if ($admin): ?>
		<details class="admin flow">
			<summary>
				<h3>Mages</h3>
			</summary>
			<table class="left-1 alternate-o">
				<colgroup>
					<col>
					<col style="width: 6ch">
					<col style="width: 6ch">
					<col style="width: 6ch">
					<col style="width: 6ch">
					<col style="width: 6ch">
				</colgroup>
				<tr>
					<th>Magerie</th>
					<th>1</th>
					<th>2</th>
					<th>3</th>
					<th>4</th>
					<th>5</th>
				</tr>
				<tr>
					<th>Total</th>
					<th>50</th>
					<th>25</th>
					<th>12</th>
					<th>6</th>
					<th>3</th>
				</tr>
				<tr>
					<td>Professeurs Université</td>
					<td></td>
					<td></td>
					<td></td>
					<td>2</td>
					<td>1</td>
				</tr>
				<tr>
					<td>Élèves Université</td>
					<td>40</td>
					<td>10</td>
					<td></td>
					<td></td>
					<td></td>
				</tr>
				<tr>
					<td>Arcania</td>
					<td></td>
					<td></td>
					<td>6</td>
					<td>2</td>
					<td>1</td>
				</tr>
				<tr>
					<td>Indépendant</td>
					<td></td>
					<td>10</td>
					<td>3</td>
					<td>2</td>
					<td>1</td>
				</tr>
				<tr>
					<td>Clandestins</td>
					<td>10</td>
					<td>5</td>
					<td>3</td>
					<td></td>
					<td></td>
				</tr>
			</table>
		</details>
	<?php endif ?>
</section>

<!-- Politique & organisation -->
<section>
	<h2>Politique &amp; organisation</h2>
	<p><b>Dirigeant de la ville :</b> Sémilis Lévère, duc d’Élmour et Prince d’Artaille</p>

	<!-- Organisation du pouvoir -->
	<details>
		<summary>
			<h3>Organisation du pouvoir</h3>
		</summary>
		<p>Le Prince est assisté par :</p>
		<ul>
			<li>Plusieurs conseillers</li>
			<li>Le Grand Conétable (ministre de la guerre)</li>
			<li>Le Premier Échevin (ministre de l'intérieur)</li>
			<li>Le Chancelier (responsable de la justice et des affaires administratives)</li>
			<li>Le Grand Argentier (ministre des finances)</li>
			<li>Le Régent des Mines &amp; Forêts</li>
		</ul>
	</details>


	<!-- Arcania -->
	<details>
		<summary>
			<h3>Arcania</h3>
		</summary>
		<p>Le quartier général de l’Arcania, la police magique, est le <i>Domus Veritas</i>, situé près du château du Prince</p>
		<p>Le Grand Mage Brener est à la tête de ses services.</p>
		<?php if ($admin): ?>
			<div class="admin flow">
				<p>L’arcania d’Imégie compte neuf mages : 1 M5 (Brener), 2 M4 (Alber dit l’<i>Olmérien</i> et Miller) et 6 M3.</p>
			</div>
		<?php endif ?>
	</details>
</section>

<!-- Économie -->
<section>
	<h2>Économie</h2>
	<p>Imégie une ville prospère</p>
	<p><b>Ressources économiques :</b> mines de fer, de plomb et d’étain, mines de charbon (fonderies dans les monts Hizrak), pêche, commerce (port), agriculture.</p>
</section>

<!-- Figures et groupes d'influence -->
<section>
	<h2>Figures et groupes d'influence</h2>

	<!-- Familles nobles -->
	<details>
		<summary>
			<h3>Familles nobles</h3>
		</summary>
		<p>250 personnes sont nobles à Imégie, répartie en 5 grandes familles.</p>
		<ul>
			<li><b>Famille Lévère :</b> la plus ancienne et la plus puissante de la région.</li>
			<li><b>Famille Borlano :</b> alliée de la famille Lévère. Ils possèdent les mines au nord de la ville. Largement impliqués dans la politique de la ville.</li>
			<li><b>Famille Abella :</b> principalement établie à Almisie, mais dont quelques membres résident à Imégie.</li>
		</ul>
	</details>

	<!-- Famille Pelren -->
	<details>
		<summary>
			<h3>Famille Pelren</h3>
		</summary>
		<p>L’une des familles bourgeoises les plus influentes d'Imégie. Cette famille est un acteur majeur du commerce maritime de la ville. En tension avec la famille Brax. Dirigée par <i>Shinor Pelren</i>.</p>
		<?php if ($admin): ?>
			<div class="admin flow">
				<p>Employeur de Nessalanta. Ils emploient une gouvernante, deux boniches et une cuisinière.</p>
				<p>La famille est liée au père Adrione, en charge d’une église dans le quartier du marché.</p>
				<p>
					<b>Shinor Pelren</b> (50 ans), le chef de famille, est l’armateur le plus riche d’Imégie, et c’est lui qui possède le plus grand entrepôt de marchandise de la ville.<br>
					Shinor a 5 enfants. Il vit avec sa femme et leur plus jeune fille (15 ans) et une autre fille (18 ans). Une autre fille (20 ans) est mariée et vit à Stomilie. Deux garçons (23 et 25 ans). L’un est plutôt désœuvré et bagarreur, l’autre, plus sérieux, aide son père dans ses affaires.
				</p>
				<p><b>Metho, le frère de Shinor</b>, est administrateur de la capitainerie du port.</p>
				<p><b>Porsar Pelren (fils ainé de Shinor)</b> s’occupe de l’entrepôt de la famille.</p>
			</div>
		<?php endif ?>
	</details>

	<!-- Famille Brax -->
	<details>
		<summary>
			<h3>Famille Brax</h3>
		</summary>
		<p>La deuxième famille bourgeoise en charge du port.</p>
		<?php if ($admin): ?>
			<div class="admin flow">
				<div class="wiki-right-element">
					<figure>
						<img src="/assets/img_paorn/maison-brax-01.jpg" />
						<figcaption>La maison Brax</figcaption>
					</figure>
				</div>
				<p>Leur maison est située dans le quartier des Consuls. Porche vers cour intérieure, entrée habitation, RdC + 2 niveaux + comble + cave, dépendance (écurie + grenier à foin)</p>
				<p>Un des membre de la famille Brax est un professeur de magie à l’université.</p>
				<p>
					<b>Suartone Brax</b>, chef de la famille Brax a le titre de <i>Bailli principal des douanes du Prince</i>. Il est en charge de la gestion des douanes du port de marchandise. C’est lui qui coordonne le contrôle des marchandises et des navires, ainsi que la collecte des taxes.
				</p>
				<p><b>Eldric Brax</b>, fils aîné de la famille. 32 ans, stature élancée et athlétique. Cheveux noirs soigneusement coiffés en arrière, révélant un front haut et des traits anguleux. Yeux perçants, d'un bleu glacial, suggérant une intelligence vive et une certaine ruse. Il porte une barbe de trois jours, ajoutant à son allure un air de mystère et de danger. Vêtu de vêtements de haute qualité, souvent dans des tons sombres, il privilégie les tenues discrètes mais élégantes, adaptées à ses activités clandestines. Une bague en argent orne son doigt, symbole de son appartenance à la haute bourgeoisie.</p>
				<p>
					For 12, Dex 12, Int 12, San 11, Per 11, Vol 10, PdV 11<br>
					Épée 14, Esquive 12, Combat MN 12<br>
					Épée longue BQ (P1d+2/T2d+1)
				</p>
			</div>
		<?php endif ?>
	</details>
</section>

<!--
<h2>Culture et société</h2>
<h2>Histoire</h2>
<h2>Particularités</h2>
-->

<!-- Aspects pratiques pour aventuriers -->
<section>
	<h2>Aspects pratiques pour aventuriers</h2>

	<!-- Alchimistes -->
	<details>
		<summary>
			<h3>Alchimistes</h3>
		</summary>
		<p>Il y en a 5 (officiels) dans toute la ville.</p>
		<ul>
			<li><b>Maître Palrod :</b> alchimiste du port qui fait commerce avec la Guilde du Crabe.</li>
			<li>Deux alchimistes dans la vieille ville</li>
			<li>Un alchimiste sur la place du Marché</li>
			<li><b>Bertinus :</b> dans le quartier des Trois Buttes. Une cinquantaine d'années, robuste et jovial, connu pour son honnêteté et la qualité de ses produits. Un peu plus cher que la moyenne.</li>
		</ul>
	</details>

	<!-- Auberges & tavernes -->
	<details>
		<summary>
			<h3>Auberges &amp; tavernes</h3>
		</summary>
		<ul>
			<li><b>Auberge de la Dunette :</b> dans le quartier du port. Auberge de standing moyen.</li>
			<li><b>Auberge du Prince :</b> dans la vieille ville. Auberge de haut standing dans le quartier de Bellevue.</li>
			<li><b><a href="wiki/paorn/auberge-vieille-tour">Auberge de la Vieille Tour</a> :</b> dans la vieille ville, quartier des Consuls.</li>
			<li><b>Auberge de la Perle :</b> quartier de Bellevue. Une auberge confortable accueillant les marchands de passage. Elle propose (très discrètement) des jeunes filles à ses clients.</li>
		</ul>
	</details>

</section>



<!--
<h4>Bandes criminelles</h4>
<p>
	Guilde du crabe (voir section détaillée)<br>
	Fraternité (voir section détaillée)
</p>

<h4>PNJ divers</h4>
<p><b>Wyrl &amp; Bukad :</b> Deux mercenaires sans conscience. Wyrl est grand et musclé (F15) avec une longue cicatrice sur le visage. Plastron+1 (RD9) + haubert de maille + casque, épée longue BQ+1 (T3d+1 P2d+1), potion de super-soin. Bukad est élancé et tout aussi grand (F13). Il porte une brigandine sur le torse et cuir rigide sur le reste du corps. Lance BQ+1 (P2d+2), potion de super-soin.</p>
<p><b>Dupwik :</b> Capitaine de la <i>Perle du Sud</i>. Associé occasionnel de Shinor.</p>
-->